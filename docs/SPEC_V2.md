...
- **Cut the FocusView "Review my day" capacity panel** (dayReview / Cmd+Shift+R — per-task hour estimates + realistic/ambitious/overloaded verdict + over-capacity warning). Hours + counts + verdict is precisely the anxiety surface being removed everywhere else. Delete the panel, its hotkey, and `/api/ai/*` day-review endpoint if unused elsewhere.
- **Cut `MorningBriefing`** (the once-a-day AI summary card) — silent start. Today already orients via the current-block header + `AgendaStrip`; a separate summary is redundant and risks the whole-pile-at-once feeling. Remove the component + `/api/ai/briefing` if unused elsewhere. Opening the app lands the user straight in their first block, no card.
